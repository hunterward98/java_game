        ...
        for (int i = 0; i < NUM_HOTBAR_SLOTS; i++) {
            ...
            if (stack != null) {
                ...
                if (item.isStackable() && stack.getQuantity() > 1) {
                    ...
                    font.draw(batch, "" + stack.getQuantity(), slotX + HOTBAR_SLOT_SIZE - 18, slotY + 14);
                }
            }

            // Draw slot number (F1-F5)
            font.setColor(Color.LIGHT_GRAY);
            font.draw(batch, "F" + (i + 1), slotX + 4, slotY + HOTBAR_SLOT_SIZE - 4);
        }

        // Draw stat bar labels and values
        font.setColor(Color.WHITE);
        font.draw(batch, "Lvl " + stats.getLevel(), barsStartX, barsY + 3 * (BAR_HEIGHT + BAR_PADDING) + BAR_HEIGHT + 2);
        font.draw(batch, "HP: " + (int)stats.getCurrentHealth() + "/" + (int)stats.getMaxHealth(),
                  barsStartX, barsY + 2 * (BAR_HEIGHT + BAR_PADDING) + BAR_HEIGHT + 2);
        font.draw(batch, "MP: " + (int)stats.getCurrentMana() + "/" + (int)stats.getMaxMana(),
                  barsStartX, barsY + (BAR_HEIGHT + BAR_PADDING) + BAR_HEIGHT + 2);
        font.draw(batch, "Stam: " + (int)stats.getCurrentStamina() + "/" + (int)stats.getMaxStamina(),
                  barsStartX, barsY + BAR_HEIGHT + 2);

        // Draw gold
        float goldX = barsStartX + barWidth + PANEL_PADDING;
        float goldY = uiY + uiHeight / 2;
        font.setColor(XP_BAR_COLOR); // Gold color
        font.draw(batch, "Gold", goldX, goldY + 20);
        font.setColor(Color.WHITE);
        font.draw(batch, "" + stats.getGold(), goldX, goldY);

        // Draw XP progress
        if (!stats.isMaxLevel()) {
            font.setColor(Color.LIGHT_GRAY);
            String xpText = (int)stats.getCurrentXP() + "/" + (int)stats.getXPRequiredForNextLevel() + " XP";
            font.draw(batch, xpText, barsStartX + barWidth - 80, barsY + 3 * (BAR_HEIGHT + BAR_PADDING) + BAR_HEIGHT + 2);
        } else {
            font.setColor(XP_BAR_COLOR);
            font.draw(batch, "MAX", barsStartX + barWidth - 40, barsY + 3 * (BAR_HEIGHT + BAR_PADDING) + BAR_HEIGHT + 2);
        }
