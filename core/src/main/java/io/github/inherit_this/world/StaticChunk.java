...
public class StaticChunk extends Chunk {
        ...
        for (int x = 0; x < Constants.CHUNK_SIZE; x++) {
            for (int y = 0; y < Constants.CHUNK_SIZE; y++) {
                ...
                if (worldTileX < 0 || worldTileX >= mapData.width ||
                    ...
                    tileLayers[x][y].add(new Tile(texture, true, TileType.VOID, TileLayer.GROUND, -1f, 0));
                } else {
                    ...
                    if (tileLayers[x][y].isEmpty()) {
                        ...
                        tileLayers[x][y].add(new Tile(texture, false, TileType.GRASS, TileLayer.GROUND, -1f, 0));
                    }
                }
            }
        }
     ...
     * Format: "tileType:layer:direction" or just "tileType" (defaults to GROUND layer)
     */
    private Tile parseTileData(String tileData) {
        ...
        // Parse tile type
        if (tileType.startsWith("stone_wall")) {
            texture = textureManager.getTexture("tiles/stone_wall.png");
            solid = true;
            type = TileType.STONE;
        } else if (tileType.startsWith("stone_")) {
            texture = textureManager.getTexture("tiles/" + tileType + ".png");
            solid = false;
            type = TileType.STONE;
        } else if (tileType.startsWith("grass_")) {
            texture = textureManager.getTexture("tiles/" + tileType + ".png");
            solid = false;
            type = TileType.GRASS;
        } else {
        ...
        return new Tile(texture, solid, type, layer, -1f, direction);
    }
            ...
            for (int y = 0; y < Constants.CHUNK_SIZE; y++) {
                ...
                if (tilesAtPosition != null) {
                    for (Tile tile : tilesAtPosition) {
                        ...
                        float yOffset = layer != null ? layer.getYOffset() : 0f;
